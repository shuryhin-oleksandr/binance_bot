...
class PriceMonitoring:
    def __init__(self, kline_manager, time_window, target_price_growth_percent, target_price_drop_percent, plot_graphic=False):
    ...
    def _analyze_kline(self, kline, min_price):
        ...
        calculated_target_price_growth_percent = ((closing_price - min_price) / min_price) * 100
        ...
        if self.high_kline:
            calculated_target_price_drop_percent = ((self.high_kline["close"] - closing_price) / self.high_kline["close"]) * 100
            kline["target_price_drop_percent"] = calculated_target_price_drop_percent
            if calculated_target_price_drop_percent >= self.target_price_drop_percent and self._is_lowest_kline(kline):
                self.low_kline = kline
                kline["status"] = "low"
                self.mid_price = math.sqrt(self.high_kline["close"] * self.low_kline["close"])
                self.mid_kline = None
                log_low_kline(kline)
                return kline
...
class HistoricalPriceMonitoring(PriceMonitoring):
    def __init__(self, kline_manager, time_window, target_price_growth_percent, target_price_drop_percent, plot_graphic, t1, t2):
        super().__init__(kline_manager, time_window, target_price_growth_percent, target_price_drop_percent, plot_graphic)
    ...
    def monitoring(self):
        ...
        chank_time_size = int(60 * 60 * 24 * 43 * 365 * TIME_STEP / 1000) # klines size ~ 0.5 Gb

        while chunk_analysis_start_time < self.analysis_end_time:
            chunk_start_time = chunk_analysis_start_time - self.time_window # get data starting from t1 - Yhr
            chunk_end_time = min(chunk_analysis_start_time + chank_time_size, self.analysis_end_time)

            klines = self.kline_manager.find_or_fetch_klines_in_range(chunk_start_time, chunk_end_time)
...
def main():
    ...
    parser.add_argument('target_price_growth_percent', type=float, help='Percentage rised threshold (X%)')
    parser.add_argument('target_price_drop_percent', type=float, help='Percentage drop threshold (Y%)')
    ...
    if args.real_time:
        monitoring = RealTimePriceMonitoring(kline_manager, args.time_window, args.target_price_growth_percent, args.target_price_drop_percent, args.plot_graphic)
    else:
        if not args.t1 or not args.t2:
            raise ValueError("Set t1 t2 interval.")
        monitoring = HistoricalPriceMonitoring(kline_manager, args.time_window, args.target_price_growth_percent, args.target_price_drop_percent, args.plot_graphic, args.t1, args.t2)
