...
def log_high_kline(kline):
    logging.info(f"High kline: (the price increased by {kline['target_price_growth_percent']}% from its peak.)")
    log_kline(kline)

def log_low_kline(kline):
    logging.info(f"Low kline: (the price has dropped by {kline['target_price_drop_percent']}%)")
    log_kline(kline)
